...
-- Output : Total number rows in each table of the schema:
...
SELECT 
		SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS ID_null_val, 
		SUM(CASE WHEN title IS NULL THEN 1 ELSE 0 END) AS title_null_val, 
		SUM(CASE WHEN year IS NULL THEN 1 ELSE 0 END) AS year_null_val,
		SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS date_published_null_val,
		SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS duration_null_val,
		SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS country_null_val,
		SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS worlwide_gross_income_null_val,
		SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS languages_null_val,
		SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS production_company_null_val
FROM movie;

-- Output : Columns in the movie table having null values
/*		country_null_val : 20
		worldwide_gross_income_null_val : 3724
		languages_null_val : 194
		production_company_null_val : 528 */
...
SELECT month(date_published), COUNT(id) as number_of_movies
FROM movie
...
-- Type your code below:
SELECT COUNT(DISTINCT id) as movies_produced, year
FROM movie
WHERE (country LIKE '%India%' or country LIKE '%USA%') AND year=2019;

-- Output : 1059 movies were produced in USA or India.
...
-- Total 13 unique genre are present in the dataset.
...
-- 4285 movies were produced in the Drama genre.
...
-- Kirkat and Love in Kilnerry have obtained rank 1 with avg_ratings as 10.0
...
-- Dream Warrior Pictures and National Theatre Live has produced the most number of hit movies with average ratings greater than 8.
-- It's ok if RANK() or DENSE_RANK() is used too
...
-- Drama genre has the maxmimum movie count of 24.
-- Drama, Comedy and Action genre are the top 3 genre having more than 1,000 votes in March 2017 in USA.
...
SELECT median_rating, COUNT(id) AS movies_count
FROM movie AS m
	INNER JOIN ratings AS r
		ON m.id = r.movie_id
...
SELECT
SUM(CASE WHEN languages LIKE '%german%' THEN total_votes END) AS german_votes,
SUM(CASE WHEN languages LIKE '%italian%' THEN total_votes END) AS italian_votes
FROM ratings AS r 
	INNER JOIN movie m 
		ON r.movie_id=m.id;
...
top_directors AS(
	...
	FROM names AS n 
		...
		INNER JOIN ratings r ON r.movie_id= g.movie_id,genre_selection
...
-- Top 2 actors are Mammootty and Mohanlal with median rating greater than equal to 8.
...
FROM top_genre
WHERE genre_rank<4),
-- from these top genre, we will select top 5 movies
top_five AS(
	...
	WHERE genre IN (SELECT genre FROM genre_select))
...
-- Parvathy Thiruvothu is on Top with total_votes of 4974 and avg_rating is 8.25
